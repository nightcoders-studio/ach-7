...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    if (['SELESAI', 'DIBATALKAN'].includes(order.status)) {
      return NextResponse.json({ error: 'Pesanan sudah selesai/dibatalkan' }, { status: 400 })
    }
    ...
    if (action === 'terima') {
      ...
      await prisma.$transaction(async (tx) => {
        const updated = await tx.order.update({
          where: { id, status: 'DIKIRIM' },
          data: { status: 'SELESAI' },
        })

        if (!updated) throw new Error('Pesanan sudah diproses sebelumnya')
